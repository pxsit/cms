(function() {
    ...
    // localStorage-based storage functions (replacing Chrome storage API)
    async function loadStorageCache() {
        const responseCacheKey = `cms_extension_${baseURL}_${user}_responseCache`;

        try {
            const data = localStorage.getItem(responseCacheKey);
            if (data) {
                const parsedData = JSON.parse(data);
                responseCache = new Map(Object.entries(parsedData));
            ...
            console.error("Error retrieving storage data:", e);
        }
    }
    ...
    async function storeStorageCache() {
        ...
        try {
            const object = Object.fromEntries(responseCache);
            localStorage.setItem(responseCacheKey, JSON.stringify(object));
        } catch (e) {
        ...
        try {
            ...
            element.innerHTML = `
                ...
                <span style="float:right">
                    <div class="cms-score-badge task_score score_${
                        currentScore == currentFullScore ? "100" :
                        currentScore > 0 ? "0_100" : "0"
                    }">
    ...
    async function initializeCMSExtension() {
        if (!isCMSPage()) {
            console.log("Not a CMS page, score enhancement disabled");
            return;
        }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeCMSExtension);
    } else {
        initializeCMSExtension();
    }
